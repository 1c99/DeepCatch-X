...
def resize_back(nii_np, data):
    # Resize back to original shape
    _, file_extension = os.path.splitext(data['path'][0])
    target_size = max(nii_np.shape)
    original_width, original_height = data['original_input_shape'][0].numpy()[0], data['original_input_shape'][1].numpy()[0]
    ...
    img_ = Image.fromarray((img_masked[:, :, 0, 0] * 255).astype(np.uint8))
    try:
        if file_extension.lower() == '.dcm':
            img_ = img_.resize((original_width, original_height), Image.LANCZOS)  # dcm
        elif file_extension.lower() == '.nii':
            img_ = img_.resize([original_height, original_width], Image.LANCZOS)  # nii
        else:
            img_ = img_.resize((original_width, original_height), Image.LANCZOS)
    except:
        if file_extension.lower() == '.dcm':
            img_ = img_.resize((original_width, original_height), Image.NEAREST)  # dcm
        elif file_extension.lower() == '.nii':
            img_ = img_.resize([original_width, original_height], Image.NEAREST)  # nii
        else:
            img_ = img_.resize((original_width, original_height), Image.NEAREST)

    nii_np = np.array(img_)
    nii_np = (nii_np/255) * (nii_np_max - nii_np_min) + nii_np_min
    return nii_np
...
def test(opt, device_num=0):
    ...
    for i, data in enumerate(dataset):
        ...
        if is_xray:
            ...
            model.float()
            with torch.no_grad():
                traced_cell = torch.jit.trace(model, (data['label'].type(torch.cuda.FloatTensor), data['inst'].type(torch.cuda.FloatTensor)) )
                generated = traced_cell(data['label'].type(torch.cuda.FloatTensor), data['inst'].type(torch.cuda.FloatTensor) )
                #generated = model((data['label'].type(torch.cuda.FloatTensor), data['inst'].type(torch.cuda.FloatTensor)))
                #generated = model.inference(data['label'].type(torch.cuda.FloatTensor), data['inst'].type(torch.cuda.FloatTensor), data['image'].type(torch.cuda.FloatTensor))
            ...
            ratio = float(opt.loadSize) / max(original_width, original_height)
            ...
            #resize ori
            nii_np_ori = resize_back(nii_np, data)
            nii = nib.Nifti1Image(nii_np_ori.astype(np.int16), affine=None)
            nii.header['pixdim'][::2] = data['pixel_spacing'][0]
            nii.header['pixdim'][1::2] = data['pixel_spacing'][1]
            nib.save(nii, opt.test_output + f'_{i}_ori_size.nii')
            #
...
def test_t12_regression(bonenii):
    return test_vbm_regression(bonenii, './checkpoints/BonenanINDVBM12_Regression.pth')

def test_l1_regression(bonenii):
    return test_vbm_regression(bonenii, './checkpoints/BonenanINDVBM13_Regression.pth')

def test_vbm_regression(heart, weight_name):
    #state = torch.load("./checkpoints/BonenanINDVBM12_Regression.pth", map_location='cuda:0')
    state = torch.load(weight_name, map_location='cuda:0')
    ...
    net = UNetRFull(n_channels=1, n_classes=n_class, model_parallelism=False, args=input_feature, isHeart=True)
    ...
    net.eval()
    # print(input_feat)
    # plt.imshow(input_img[0,0].cpu().detach().numpy())
    # plt.show()
    with torch.no_grad():
    ...
    torch.cuda.empty_cache()
    #print(heartarea, target_resize, result, pixel_spacing[0])
    print(result)
    return result

    
def test_heart_regression(heart, heartarea):
    state = torch.load("./checkpoints/heartregression.pth", map_location='cuda:0')
    ww = state.get('ww')
    wl = state.get('wl')
    n_class = state.get('n_class')
    input_feature = state.get('input_feature')
    ...
    # print(heartarea, sex, age)
    input_feat = torch.from_numpy(np.array([[heartarea]], dtype=np.float32))
    input_img = torch.from_numpy(input_norm[np.newaxis, np.newaxis]).float()
    input_img = input_img.cuda()
