...
from test import test, color_overlay, test_t12_regression, test_l1_regression, print_gpu_stats
import os
...
def predict(input, input_dir, output_dir):
    ...
    try:
        ...
        data["message"] = 'TiSepX Prediction Exception: ' + str(e)
        data["success"] = False
    ...
    if data["success"]:
        ...
        l1nii = os.path.join(output_dir, l1output_filename + '.nii')
    ...
    if data["success"]:
        input_filename = os.path.join(input_dir, input)
        boneoutput_filename = input + '_bonenan.png'
        
        bonenii = os.path.join(output_dir, boneoutput_filename + '.nii')
        bonepath = os.path.join(output_dir, boneoutput_filename)
        print(input_filename, boneoutput_filename)    
        
        opt.test_input = input_filename
        opt.test_output = bonepath
        opt.get_heart_area = True
        opt.hn = False
        opt.output_min_max = "-1024,1476"
        opt.checkpoint_path = "./checkpoints/xray2bonenan.pth"
        opt.threshold = -1015
        opt.save_input = True
        opt.profnorm = True
        opt.check_xray = False
        ...
        try:
            bonenanarea, is_xray, _, _, _, original_width, original_height = test(opt)
        except Exception as e:
            is_xray = False
            bonenanarea = -2
            heartoutput_filename = ''
    ...
    #'''
    #Regression
    #T12: BonenanINDVBM12_Regression.pth
    
    if data["success"]:
        try:
            t12hu = test_t12_regression(bonenii)
            data['t12meanhu'] = t12hu   
        except Exception as e:
            is_xray = False
            #heartvolume = -2
            data["message"] = 'TiSepX Prediction Error: ' + str(e)
            data["success"] = False
    ...
    #L1: BonenanINDVBM13_Regression.pth            
    if data["success"]:
        try:
            l1hu = test_l1_regression(bonenii)
            data['l1hu'] = l1hu            
        except Exception as e:
            is_xray = False
            #heartvolume = -2
